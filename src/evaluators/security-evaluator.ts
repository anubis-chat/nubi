...
export const securityEvaluator: Evaluator = {
  ...
  description: "Filters malicious requests and protects against prompt injection and spam",
  ...
  handler: async (
    ...
    try {
      ...
      const securityResult = await securityFilter.processMessage(userId, messageText);
      ...
      if (!securityResult.allowed) {
        ...
        return {
          success: false,
          text: securityResult.response || "Security filter blocked this request",
          values: {
            blocked: true,
            violationType: securityResult.violationType,
            securityResponse: securityResult.response,
          },
          error: new Error(`Security violation: ${securityResult.violationType}`),
        };
      }

      // Message passed security checks
      logger.debug(`[SECURITY_EVALUATOR] Message from ${userId} passed security checks`);
