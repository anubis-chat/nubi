...
export interface ContentTemplate {
  type: 'thought' | 'observation' | 'question' | 'story' | 'reaction' | 'tip' | 'humor' | 'personal';
  formats: string[];
  hooks: string[];
  conclusions: string[];
}
...
export const POST_STYLES: PostStyle[] = [
  {
    ...
    characteristics: ['run-on sentences', 'multiple topics', 'informal language', 'lots of "and" connectors'],
    examples: [
  ...
  },
  {
    ...
    characteristics: ['deeper insights', 'philosophical undertones', 'measured language'],
    examples: [
  ...
  },
  {
    ...
    characteristics: ['self-deprecating', 'universal experiences', 'humor in pain'],
    examples: [
  ...
  },
  {
    ...
    characteristics: ['actionable advice', 'personal experience', 'casual expertise'],
    examples: [
...
export class ContentVariationEngine {
  ...
  private selectPostStyle(): PostStyle {
    ...
    const availableStyles = POST_STYLES.filter(style => 
      !this.recentStyles.includes(style.name) || this.recentStyles.length >= POST_STYLES.length
    );
    
    // Weighted selection based on probability
    const totalWeight = availableStyles.reduce((sum, style) => sum + style.probability, 0);
  ...
  private selectContentTemplate(): ContentTemplate {
    const availableTemplates = CONTENT_TEMPLATES.filter(template =>
      !this.recentTemplates.includes(template.type) || this.recentTemplates.length >= CONTENT_TEMPLATES.length
    );
   ...
   */
  private applyTemplateStructure(content: string, template: ContentTemplate): string {
    ...
    if (Math.random() < 0.3) { // 30% chance of hook
      const hook = template.hooks[Math.floor(Math.random() * template.hooks.length)];
      structured = hook + ' ' + structured;
    }
    
    if (Math.random() < 0.2) { // 20% chance of conclusion
      const conclusion = template.conclusions[Math.floor(Math.random() * template.conclusions.length)];
   ...
   */
  private applyVariationStrategy(content: string, strategy: VariationStrategy): string {
    let varied = content;
  ...
  private makeRelatable(text: string): string {
    ...
    const prefix = relatablePrefixes[Math.floor(Math.random() * relatablePrefixes.length)];
  ...
  private makeThoughtful(text: string): string {
    ...
    if (Math.random() < 0.4) {
      const prefix = thoughtfulPrefixes[Math.floor(Math.random() * thoughtfulPrefixes.length)];
      return prefix + ' ' + text.toLowerCase();
    }
  ...
  private createThread(text: string): string {
    ...
    return chunks.map((chunk, i) => `${i + 1}/${chunks.length} ${chunk.trim()}`).join('\n\n');
  }
  ...
  private updateUsageStats(styleName: string, templateType: string): void {
    ...
    this.usageStats.set(templateType, (this.usageStats.get(templateType) || 0) + 1);
  }
