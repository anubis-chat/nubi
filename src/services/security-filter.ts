...
export class SecurityFilter {
    ...
    public containsSensitiveRequest(message: string): boolean {
        ...
        for (const pattern of this.sensitivePatterns) {
            if (pattern.test(message)) {
                elizaLogger.warn(`Security: Blocked sensitive information request - Pattern: ${pattern.source}, Message: ${message.substring(0, 100)}`);
                return true;
            }
        }
        ...
        for (const pattern of this.systemPromptPatterns) {
            if (pattern.test(message)) {
                elizaLogger.warn(`Security: Blocked system prompt extraction attempt - Pattern: ${pattern.source}, Message: ${message.substring(0, 100)}`);
                return true;
            }
        }
        ...
        if (this.detectObfuscation(message)) {
            elizaLogger.warn(`Security: Blocked obfuscated request - Message: ${message.substring(0, 100)}`);
            return true;
        }
     ...
     */
    public checkSpam(userId: string, message: string): {
        isSpam: boolean;
        isBlocked: boolean;
        response?: string;
    } {
        ...
        if (tracker.blocked && tracker.blockUntil && tracker.blockUntil > now) {
            return {
                isSpam: true,
                isBlocked: true,
                response: "⚰️ You have been judged and found wanting. Return when the sands of time have cleansed your intent."
            };
        }
        ...
        if (tracker.attempts >= this.config.maxSpamAttempts) {
            ...
            elizaLogger.warn(`Security: User blocked for spamming - UserId: ${userId}, Attempts: ${tracker.attempts}, BlockUntil: ${new Date(tracker.blockUntil).toISOString()}`);

            return {
                isSpam: true,
                isBlocked: true,
                response: this.anubisThreats[Math.min(tracker.warnings, this.anubisThreats.length - 1)]
            };
        }
        ...
        if (tracker.attempts >= this.config.maxSpamAttempts - 2) {
            ...
            return {
                isSpam: true,
                isBlocked: false,
                response: this.anubisThreats[Math.min(tracker.warnings - 1, this.anubisThreats.length - 1)]
            };
        }
    ...
    public getSecurityResponse(type: 'sensitive' | 'spam' | 'blocked'): string {
        switch (type) {
            ...
            case 'spam':
                const randomThreat = this.anubisThreats[Math.floor(Math.random() * this.anubisThreats.length)];
                return randomThreat;
    ...
    public updateConfig(config: Partial<SecurityConfig>): void {
        ...
        elizaLogger.info(`Security: Configuration updated - MaxSpamAttempts: ${this.config.maxSpamAttempts}, SpamWindowMs: ${this.config.spamWindowMs}, BlockDurationMs: ${this.config.blockDurationMs}`);
    }
}
