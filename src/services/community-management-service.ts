...
export class CommunityManagementService extends Service {
  ...
  capabilityDescription = "Community engagement tracking and relationship management";
  ...
  static async start(runtime: IAgentRuntime): Promise<CommunityManagementService> {
   ...
   */
  async trackUserInteraction(message: Memory, sentiment?: number): Promise<void> {
    try {
      const userId = message.entityId;
      const messageText = message.content.text || "";
      
      // Update user engagement
      const existing = this.userEngagements.get(userId) || this.createNewUserEngagement(userId);
      ...
      existing.topics = [...new Set([...existing.topics, ...topics])].slice(-20); // Keep last 20 topics
      ...
      logger.warn("[COMMUNITY_MANAGEMENT_SERVICE] Failed to track user interaction:", error);
    }
  }
  ...
  private async updateCommunityMetrics(): Promise<void> {
    try {
      ...
      this.communityMetrics.engagementScore = totalUsers > 0 
        ? Math.round((activeUsers / totalUsers) * 100 + (dailyMessages / totalUsers) * 10)
        : 0;
      ...
      logger.debug(`[COMMUNITY_MANAGEMENT_SERVICE] Updated metrics: ${this.communityMetrics.engagementScore}% engagement, mood: ${this.communityMetrics.communityMood}`);
      
    } catch (error) {
      logger.warn("[COMMUNITY_MANAGEMENT_SERVICE] Failed to update community metrics:", error);
    }
  }
  ...
  private extractTopics(text: string): string[] {
    ...
    if (lowerText.match(/trading|price|market|pump|dump/)) topics.push('trading');
    ...
    // Community topics
    if (lowerText.match(/community|together|help|support/)) topics.push('community');
   ...
   */
  private calculateRelationshipLevel(engagement: UserEngagement): UserEngagement['relationshipLevel'] {
    ...
    if (messageCount >= 20 && daysSinceLastSeen <= 14) return 'community_member';
  ...
  private calculateCommunityMood(): string {
    const sentiments = Array.from(this.userEngagements.values()).map(u => u.sentiment);
  ...
  async getCommunityInsights(): Promise<{
    ...
    const valuedMembers = Array.from(this.userEngagements.values())
      .filter(u => u.relationshipLevel === 'valued_member').length;
  ...
  async stop(): Promise<void> {
    logger.info("[COMMUNITY_MANAGEMENT_SERVICE] Stopping community management service");
