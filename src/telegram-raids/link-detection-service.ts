...
export class LinkDetectionService {
  ...
  async validateAndExtractTweetData(url: string): Promise<TweetData | null> {
    try {
      ...
      const tweetIdMatch = cleanUrl.match(/(?:twitter\.com|x\.com)\/([a-zA-Z0-9_]+)\/status\/(\d+)/);
    ...
    try {
      ...
      for (const link of links) {
        ...
        if (tweetData?.isValid) {
          logger.info(`Valid X link detected by @${username}: ${tweetData.url}`);
        } else {
  ...
  async batchValidateUrls(urls: string[]): Promise<TweetData[]> {
    ...
    for (let i = 0; i < urls.length; i += batchSize) {
      ...
      const batchPromises = batch.map(url => this.validateAndExtractTweetData(url));
      ...
      try {
        ...
        results.forEach((result, index) => {
          if (result.status === 'fulfilled' && result.value?.isValid) {
            ...
            logger.error(`Failed to validate URL ${batch[index]}:`, result.reason);
          }
        });
